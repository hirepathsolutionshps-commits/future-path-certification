    ...
    const handlePayAndSubmit = async () => {
      ...
      if (!window.PaystackPop) {
        try {
          ...
          toast.error(
            "Paystack could not load. Please check your connection and try again.",
          );
          return;
        }
      }
      ...
      if (!window.PaystackPop) {
        toast.error(
          "Paystack is unavailable. Please try again in a moment.",
        );
        return;
      }

      const ref = `hps-${Date.now()}-${Math.random()
        .toString(36)
        .slice(2, 8)}`;
      ...
      const handler = window.PaystackPop.setup({
        ...
        callback: (response) => {
          ...
          (async () => {
            try {
              // Verify payment server-side, save to Supabase, and send email
              const res = await fetch("/api/verify-payment", {
                ...
                body: JSON.stringify({
                  ...
                  cohort_name:
                    cohort?.name ?? "Healthcare VA Program",
                }),
              });
              ...
              toast.error(
                "Something went wrong. Contact us on WhatsApp — your payment is safe.",
              );
            } finally {
              setSubmitting(false);
            }
        ...
        <div className="mx-auto max-w-2xl px-5 py-20 sm:px-8 sm:py-28">
          <Reveal className="mb-10 text-center">
            ...
            <p className="mx-auto mt-4 max-w-md text-graphite">
              Three quick steps, then pay securely with Paystack to confirm
              your seat.
            </p>
          </Reveal>
              ...
              {done ? (
                <div className="py-8 text-center">
                  ...
                  <p className="mx-auto mt-3 max-w-sm text-graphite">
                    Welcome, {form.name.split(" ")[0]}! A confirmation email
                    is on its way. Our team will reach out on WhatsApp within
                    24 hours with your onboarding details.
                  </p>
                    ...
                    {step === 2 && (
                      <motion.div key="s2" {...stepAnim}>
                        ...
                        <p className="mt-4 text-center text-xs text-graphite/70">
                          You will be taken to a secure Paystack payment page
                          after clicking below.
                        </p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                  ...
                  <div className="mt-8 flex items-center justify-between gap-3">
                    ...
                    {step < 2 ? (
                      ...
                      <CtaButton
                        ...
                        variant="gold"
                        disabled={
                          submitting || !form.schedule_type
                        }
                      >
                        {submitting
                          ? "Processing…"
                          : "Pay and Confirm Seat"}
                      </CtaButton>
                    )}
                  </div>
                </>
